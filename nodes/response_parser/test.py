...
class TestResponseParser(ResponseParser):
    ...
    def validate_revise_response(parsed_response):
        ...
        if SharedKeys.FUNCTION_SUGGESTION in parsed_response:
            if isinstance(parsed_response[SharedKeys.FUNCTION_SUGGESTION], list):
                for func in parsed_response[SharedKeys.FUNCTION_SUGGESTION]:
